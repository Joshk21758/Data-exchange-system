...
export default async function MinistryOfForeignAffairsPage() {
  //get foreign affairs collection
  const foreignAffairsCollection = await getCollection(
    "foreign-affairs-collection"
  );
  const posts = await foreignAffairsCollection
    .find()
    .sort({ $natural: -1 })
    .toArray();
  ...
  return (
    <div className="grid flex-1 items-start gap-4 md:gap-8">
      <Card>
        <CardHeader>
          <CardTitle>Ministry of Foreign Affairs - Applications</CardTitle>
          <CardDescription>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Review and Manage data requests from other ministries
          </CardDescription>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {requests.map((req) => (
                  <TableRow key={req._id}>
                    ...
                    <TableCell>
                      {req._id.getTimestamp().toDateString()}
                    </TableCell>
                    <TableCell>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
